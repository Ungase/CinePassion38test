...
abstract class controleur
{
    ...
    // Le module après "traitement"
    protected $page;
    
    // La page après "traitement"
    protected $action;
    
    // L' action après "traitement"
    protected $donnees = array();
    
    // Le tableau où sont stockées les données pour la vue
    ...
    protected function setDonnees()
    {
        ...
        // ===============================================================================================================
        // Test si l'utilisateur veut se connecter ou est déjà connecter
        // ===============================================================================================================
        ...
        $this->publicRsaKey = (new modelRSA)->getPublicKeyRsa($this->numRsa);
        // ===============================================================================================================
    ...
    public function __set1($cle, $valeur)
    {
     ...
     *            : le nom et l'extention du fichier ou librairie a charger
     * @return string : ligne html pour recuperer le fichier passer en parametre
     * @author Fabrice Gonçalves
     */
    protected function IntLien($fichier)
    {
        if (! strpos($fichier, ".")) {
            $chaine = configuration::get("dossierLibrairie");
            $resultat = "<link rel='stylesheet' type='text/css' href='" . $chaine . $fichier . "/css/" . $fichier . ".css' />
                         <script type='text/javascript' src='" . $chaine . $fichier . "/js/" . $fichier . ".js'></script>";
        } else {
            $extension = pathinfo($fichier, PATHINFO_EXTENSION);
            $Tableau = explode("!", configuration::get("dossier" . $extension));
            foreach ($Tableau as $string) {
                if (file_exists($string . $fichier)) {
                    $fichier = $string . $fichier;
                }
            }
            if ($extension == "css") {
                
                $resultat = "<link rel='stylesheet' type='text/css' href='" . $fichier . "' />\n";
            } else {
                
                $resultat = "<script type='text/javascript' src='" . $fichier . "'></script>\n";
            }
        }
        return $resultat;
    }
    ...
    {
        if (!isset($this->btNavig)) {
            $this->btNavig = (object) array(
                "SectionMin" => $this->section,
                "SectionMax" => $this->nbSection,
                "SectionPrecedente" => $this->section - 1,
                "SectionSuivante" => $this->section + 1
            );
        }
        
        return new navigation($this->module, $this->page, $this->action, $this->section, $this->nbSection, $this->btNavig);
    }
    ...
    protected function getNbSections($nbFilms) // a mettre dans la classe mere
    {
        return round(($nbFilms) / configuration::get("nbLignesSection"), 0, PHP_ROUND_HALF_UP);
    }
    
    /**
     * @param array des valeurs voulu
     * @return valeur du tableau, type dependant des valeurs rentré dans le tableau
     */
    protected function alea($array){
